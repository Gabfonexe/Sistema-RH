...
public class VagaServiceImpl implements VagaService{
  ...
  /* 
  @Override
  public CandidatoModel adicionarCandidato(CandidatoModel candidato) {
    try {
      Optional<CandidatoModel> candidatos = candidatoRepository.findByCpf(candidato.getCpf());
      var candidatoAceito = candidatos.get();
      if(candidatoAceito == null){
        throw new NoSuchElementExceptionVaga("Sem candidato confirmado");
    }
      return vagaRepository.save(candidatoAceito);
    } catch (Exception e) {
  ...
  public List<VagaModel> adicionarCandidatos(VagaModel vaga){
    ...
    var indice = 0;
    for(CandidatoModel candidato : candidatos){
      if(candidatos.get(indice).getAtuacao() == vagas.get(indice).getAtuacao() && candidatos.get(indice).getExperiencia() == vagas.get(indice).getExperiencia() && candidatos.get(indice).getSalario() == vagas.get(indice).getSalario()){
        //List<CandidatoModel> candidatosAceitos = new ArrayList<>();
        //candidatosAceitos.add((CandidatoModel) candidato);
        List<CandidatoModel> vagasConcluidas = new ArrayList<>();
        vagasConcluidas.add(candidato);
        vagas.get(indice).setCandidatos(vagasConcluidas); 
      }
      indice++;
    }
  ...
  @Override
  public List<VagaModel> adicionarCandidatos2(VagaModel vaga) {
      List<CandidatoModel> candidatos = candidatoRepository.findAll();
      List<VagaModel> vagas = vagaRepository.findAll();
  
      for (CandidatoModel candidato : candidatos) {
          for (VagaModel vagaAtual : vagas) {
              if (candidato.getAtuacao().equalsIgnoreCase(vagaAtual.getAtuacao())
                      && candidato.getExperiencia()==(vagaAtual.getExperiencia())
                      && candidato.getSalario().equals(vagaAtual.getSalario())) {
                  if (vagaAtual.getCandidatos() == null) {
                      vagaAtual.setCandidatos(new ArrayList<>());
                  }
      ...
      // Considerando que você quer atualizar as vagas no banco de dados:
  ...
  @Override
  public List<VagaModel> contratar(VagaModel vaga) {
    List<VagaModel> vagas = vagaRepository.findAll();
    List<CandidatoModel> candidatos = candidatoRepository.findAll();
    for(VagaModel vagaAtual : vagas){
      if(!vagaAtual.getCandidatos().isEmpty()){
        for(CandidatoModel candidato : candidatos){
          if(vagas.contains(candidato)){
            List<CandidatoModel> possiveisCandidatos = new ArrayList<>();
            possiveisCandidatos.add(candidato);
            for(CandidatoModel candidatoProvavel : possiveisCandidatos){
              if(candidatoProvavel.getAtuacao().equals(vagaAtual.getAtuacao())){
                List<CandidatoModel> contratado = new ArrayList<>();
                vagas.get(0).
              }
